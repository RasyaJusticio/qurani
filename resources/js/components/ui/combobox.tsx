...
const Combobox: React.FC<ComboboxProps> = ({
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent className={`w-[200px] p-0 bg-white text-black dark:text-gray-200 dark:bg-gray-700`}>
                <Command className="bg-white text-black dark:text-gray-200 dark:bg-gray-700">
                    <CommandInput placeholder={searchPlaceholder} className="dark:placeholder:text-gray-400" />
                    <CommandList>
                        <CommandEmpty>{notFoundText}</CommandEmpty>
                        <CommandGroup>
                            {options.map((option) => (
                                <CommandItem
                                    className={`bg-white text-black dark:bg-gray-700 ${cn(
                                        "",
                                        value === option.value ? "text-[#3991D5]" : "dark:text-gray-200"
                                    )}`}
                                    key={option.value}
